...
public class EditinfUserAction implements Action {
	...
	public String execute(HttpServletRequest request,
		...
		try {
			...
			if (user.getUsername().equals(username)||DaoFactory.getUserDaoInstance().queryByName(username).getUid()==0) {
				System.out.println("DaoFactory.getUserDaoInstance().queryByName(username).getUid():"+DaoFactory.getUserDaoInstance().queryByName(username).getUid());
				System.out.println("用户名未注册");
				System.out.println("user.getEmail().equals(email):"+user.getEmail().equals(email));
				System.out.println("DaoFactory.getUserDaoInstance().queryByEmail(email).getUid()："+DaoFactory.getUserDaoInstance().queryByEmail(email).getUid());
				if(user.getEmail().equals(email)||DaoFactory.getUserDaoInstance().queryByEmail(email).getUid()==0){
					System.out.println("邮箱未使用");
					if (DaoFactory.getUserDaoInstance().editInf(uid, username, email)==1) {
